  ...
  return (
    <div className="relative">
      <input
        type={type}
        className={cn(
          'flex h-9 w-full rounded-md bg-transparent px-3 py-1 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-zinc-700 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-800 dark:placeholder:text-zinc-400',
          className
        )}
        ref={ref}
        {...props}
      ...
      {dropdownVisible && list && options && options.length > 0 && props.value && (
        <div className="bg-[#182D32] border border-input_border flex flex-col text-white_secondary rounded mt-2 absolute top-12 w-full" id={list}>
